...
# ------------------------------
print("=" * 60)
print("GPU CHECK: PYTORCH GPU DETECTION")
print("=" * 60)
...
if torch.cuda.is_available():
    ...
    try:
        ...
        print(f"   [OK] GPU test passed!")
        print(f"\n   [GPU] PyTorch will use GPU for LSTM training!")

    except Exception as e:
        print(f"   [WARN] GPU test failed: {str(e)[:100]}")
        print(f"   [INFO] Falling back to CPU mode")
        USE_GPU = False
        DEVICE = torch.device("cpu")
else:
    print(f"\n[WARN] NO GPU DETECTED")
    print(f"   PyTorch will use CPU only")

if not USE_GPU:
    print(f"\n[CPU] Running in CPU mode")
    print(f"   Training will be slower but still works fine")

print(f"\n[DEVICE] Device set to: {DEVICE}")
print("=" * 60)
...
output_dir = _base_dir / "ml_lstm_results"
print(_base_dir)
print(output_dir)
...
CONFIG = {
    ...
    # Model settings
    'method': 'lstm',               # 'lstm' = Long Short-Term Memory neural network
...
print(f"Method:               {CONFIG['method'].upper()} (LSTM Neural Network)")
...
# %%
# ------------------------------
# STEP 3.8: Define PyTorch LSTM Model
# ------------------------------
print("\n" + "=" * 60)
print("STEP 3.8: DEFINING PYTORCH LSTM MODEL")
print("=" * 60)

class LSTMRegressor(nn.Module):
    """
    PyTorch LSTM model for return prediction.
    ...
    """
    def __init__(self, input_size, lstm_units=64, dropout_rate=0.2):
...
print("\n[OK] PyTorch LSTM model defined!")
print("   - LSTMRegressor: LSTM + Dropout + Dense + Output")
print("   - train_model: Training with early stopping")
...
# %%
# ------------------------------
# STEP 4: Implement Hyperparameter Grid for LSTM
# ------------------------------
print("\n" + "=" * 60)
print("STEP 4: HYPERPARAMETER GRID FOR LSTM")
print("=" * 60)
...
def get_hyperparameter_grid(method):
    ...
    -----------
    method : str
        ML method ('lstm' for Long Short-Term Memory neural network)
    ...
    if method == 'lstm':
        # Hyperparameters for LSTM Neural Network (simplified grid - 8 combinations)
        grid = {
            'lstm_units': [64, 128],               # Number of LSTM units in hidden layer
            'dropout_rate': [0.2, 0.3],           # Dropout rate for regularization
            'learning_rate': [0.001, 0.01],       # Learning rate for Adam optimizer (slow vs fast)
            'batch_size': [64],                    # Batch size for training
            'epochs': [100]                        # Number of training epochs (with early stopping)
        }
    else:
...
if CONFIG['method'] == 'lstm':
    ...
    lstm_units_values = sorted(set([p['lstm_units'] for p in hyperparameter_grid]))
    dropout_rate_values = sorted(set([p['dropout_rate'] for p in hyperparameter_grid]))
    learning_rate_values = sorted(set([p['learning_rate'] for p in hyperparameter_grid]))
    batch_size_values = sorted(set([p['batch_size'] for p in hyperparameter_grid]))
    epochs_values = sorted(set([p['epochs'] for p in hyperparameter_grid]))

    print(f"  lstm_units (LSTM layer size):    {lstm_units_values}")
    print(f"  dropout_rate (regularization):   {dropout_rate_values}")
    print(f"  learning_rate (optimizer):       {learning_rate_values}")
    print(f"  batch_size (training batch):     {batch_size_values}")
    print(f"  epochs (max training epochs):    {epochs_values}")

print()
print("First 5 combinations to try:")
print("-" * 80)
print("  #  | lstm_units | dropout | learn_rate | batch | epochs")
print("-" * 80)

for i, params in enumerate(hyperparameter_grid[:5], 1):
    print(f"  {i:2d} | {params['lstm_units']:10d} | {params['dropout_rate']:7.2f} | "
          f"{params['learning_rate']:10.4f} | {params['batch_size']:5d} | {params['epochs']:6d}")
...
print("-" * 50)
print()
print("1. lstm_units (LSTM Layer Size):")
print("   - Number of memory cells in LSTM layer")
print("   - More units = More capacity to learn patterns")
print("   - Too many = Overfitting + slow training")
print("   - Too few = Underfitting")
print()
print("2. dropout_rate (Regularization):")
print("   - Randomly drops neurons during training")
print("   - Helps prevent overfitting")
print("   - Higher rate = More regularization")
print("   - Typical range: 0.1 to 0.5")
print()
print("3. learning_rate (Optimizer Step Size):")
print("   - Controls how fast the model learns")
print("   - Lower rate = Slower, more stable convergence")
print("   - Higher rate = Faster, but may overshoot optimum")
print("   - Typical range: 0.0001 to 0.01")
print()
print("4. batch_size (Training Batch):")
print("   - Number of samples per gradient update")
print("   - Larger batches = More stable gradients, faster")
print("   - Smaller batches = More noise, better generalization")
print()
print("5. epochs (Training Iterations):")
print("   - Maximum training epochs (with early stopping)")
...
def run_cross_validation(data, k, config, feature_cols):
    ...
    for i, params in enumerate(tunegrid):
        ...
        if config['method'] == 'lstm':
            ...
            X_train_tensor = torch.FloatTensor(X_train_scaled).unsqueeze(1)  # Add timestep dimension
            y_train_tensor = torch.FloatTensor(y_train.values)
            X_val_tensor = torch.FloatTensor(X_validation_scaled).unsqueeze(1)
            y_val_tensor = torch.FloatTensor(y_validation.values)
        ...
        if (i + 1) % 5 == 0 or (i + 1) == len(tunegrid):
            ...
            print(f"  Completed {i+1:2d}/{len(tunegrid)} | "
                  f"units={params['lstm_units']:3d}, lr={params['learning_rate']:.4f}, dropout={params['dropout_rate']:.2f} | "
    ...
    print("-" * 80)
    print("Rank | lstm_units | dropout | learn_rate | batch | epochs | MSE       | R²")
    print("-" * 80)

    for rank, (idx, row) in enumerate(cv_results.head(3).iterrows(), 1):
        print(f"  {rank}  | {row['lstm_units']:10.0f} | {row['dropout_rate']:7.2f} | "
              f"{row['learning_rate']:10.4f} | {row['batch_size']:5.0f} | {row['epochs']:6.0f} | "
...
print("     a. Normalize features using StandardScaler")
print("     b. Build and train LSTM model on TRAIN data")
print("     c. Predict on VALIDATION data")
print("     d. Calculate MSE and R² scores")
...
def run_prediction(data, k, config, feature_cols):
    ...
    print(f"   lstm_units:    {int(best_params['lstm_units'])}")
    ...
    if config['method'] == 'lstm':
        ...
        # Create DataLoaders (no validation set for final training)
        train_dataset = TensorDataset(X_train_tensor, y_train_tensor)
        train_loader = DataLoader(train_dataset, batch_size=int(best_params['batch_size']), shuffle=True)

        # Create a dummy validation loader (use last batch of training as validation for early stopping)
        val_loader = DataLoader(train_dataset, batch_size=int(best_params['batch_size']), shuffle=False)
...
USE_PARALLEL = False  # [WARN] Set to False when using GPU (TensorFlow allocates GPU memory per process)
                      # Set to True for CPU parallel processing (3-4x faster, but no GPU acceleration)
...
if len(cv_files) == 0:
    ...
    print("\n" + "-"*110)
    print(f"{'Year':<6} | {'Counter':<8} | {'Units':<6} | {'Dropout':<8} | {'LR':<8} | {'Batch':<6} | {'Epochs':<6} | {'R²':<10} | {'MSE':<12}")
    print("-"*110)
    for idx, row in best_results.iterrows():
        print(f"{int(row['year']):<6} | {int(row['counter']):<8} | {int(row['lstm_units']):<6} | "
            f"{row['dropout_rate']:<8.2f} | {row['learning_rate']:<8.4f} | "
            f"{int(row['batch_size']):<6} | {int(row['epochs']):<6} | "
    ...
    print("\n1. LSTM UNITS (in best models):")
    units_counts = best_results['lstm_units'].value_counts().sort_index()
    for units, count in units_counts.items():
        pct = count / len(best_results) * 100
        print(f"   {int(units):3d}: {count:2d} times ({pct:5.1f}%)")
    print(f"   Mean: {best_results['lstm_units'].mean():.1f}")

    print("\n2. DROPOUT RATE (in best models):")
    dropout_counts = best_results['dropout_rate'].value_counts().sort_index()
    for dropout, count in dropout_counts.items():
        pct = count / len(best_results) * 100
        print(f"   {dropout:.2f}: {count:2d} times ({pct:5.1f}%)")
    print(f"   Mean: {best_results['dropout_rate'].mean():.3f}")

    print("\n3. LEARNING RATE (in best models):")
    lr_counts = best_results['learning_rate'].value_counts().sort_index()
    for lr, count in lr_counts.items():
        pct = count / len(best_results) * 100
        print(f"   {lr:.4f}: {count:2d} times ({pct:5.1f}%)")
    print(f"   Mean: {best_results['learning_rate'].mean():.4f}")

    print("\n4. BATCH SIZE (in best models):")
    batch_counts = best_results['batch_size'].value_counts().sort_index()
    for batch, count in batch_counts.items():
        pct = count / len(best_results) * 100
        print(f"   {int(batch):3d}: {count:2d} times ({pct:5.1f}%)")
    print(f"   Mean: {best_results['batch_size'].mean():.1f}")

    print("\n5. EPOCHS (in best models):")
    epochs_counts = best_results['epochs'].value_counts().sort_index()
    for epochs, count in epochs_counts.items():
        pct = count / len(best_results) * 100
        print(f"   {int(epochs):3d}: {count:2d} times ({pct:5.1f}%)")
    print(f"   Mean: {best_results['epochs'].mean():.1f}")

    print("\n" + "="*80)
    print("TOP 10 BEST PERFORMING CONFIGURATIONS (by MSE, across all periods)")
    print("="*80)
    top10 = best_results.nsmallest(10, 'mse')[['year', 'counter', 'lstm_units', 'dropout_rate', 'learning_rate', 'batch_size', 'epochs', 'mse', 'r2_score']]
    ...
    print("\nBy LSTM UNITS (lower MSE = better):")
    units_avg = full_data.groupby('lstm_units')['mse'].mean().sort_values(ascending=True)
    for units, mse in units_avg.items():
        print(f"   {int(units):3d}: {mse:.6f}")

    print("\nBy DROPOUT RATE (lower MSE = better):")
    dropout_avg = full_data.groupby('dropout_rate')['mse'].mean().sort_values(ascending=True)
    for dropout, mse in dropout_avg.items():
        print(f"   {dropout:.2f}: {mse:.6f}")
    ...
    for lr, mse in lr_avg.items():
        print(f"   {lr:.4f}: {mse:.6f}")

    print("\nBy BATCH SIZE (lower MSE = better):")
    batch_avg = full_data.groupby('batch_size')['mse'].mean().sort_values(ascending=True)
    for batch, mse in batch_avg.items():
        print(f"   {int(batch):3d}: {mse:.6f}")

    print("\nBy EPOCHS (lower MSE = better):")
    epochs_avg = full_data.groupby('epochs')['mse'].mean().sort_values(ascending=True)
    for epochs, mse in epochs_avg.items():
        print(f"   {int(epochs):3d}: {mse:.6f}")
    ...
    print(f"   LSTM Units:    {int(best_results['lstm_units'].mode()[0])} (wins {units_counts.max()} times)")
    print(f"   Dropout Rate:  {best_results['dropout_rate'].mode()[0]:.2f} (wins {dropout_counts.max()} times)")
    print(f"   Learning Rate: {best_results['learning_rate'].mode()[0]:.4f} (wins {lr_counts.max()} times)")
    print(f"   Batch Size:    {int(best_results['batch_size'].mode()[0])} (wins {batch_counts.max()} times)")
    print(f"   Epochs:        {int(best_results['epochs'].mode()[0])} (wins {epochs_counts.max()} times)")

    print(f"\n[STATS] Most Consistent Hyperparameters (by avg MSE - lower is better):")
    print(f"   LSTM Units:    {int(units_avg.idxmin())} (avg MSE: {units_avg.min():.6f})")
    print(f"   Dropout Rate:  {dropout_avg.idxmin():.2f} (avg MSE: {dropout_avg.min():.6f})")
    print(f"   Learning Rate: {lr_avg.idxmin():.4f} (avg MSE: {lr_avg.min():.6f})")
    print(f"   Batch Size:    {int(batch_avg.idxmin())} (avg MSE: {batch_avg.min():.6f})")
    print(f"   Epochs:        {int(epochs_avg.idxmin())} (avg MSE: {epochs_avg.min():.6f})")

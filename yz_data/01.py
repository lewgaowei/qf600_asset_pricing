...
base_directory = "/Users/ngchunyue/Documents/MQF/QF600 Asset Pricing/Project/copy_qf600/qf600_asset_pricing/yz_data"

def end_step(name):
    print(f"--- End of step: {name} ---", flush=True)

START_YEAR = 2015
require_wrds_load = True # adjust accordingly

# --------------------
# 1. Load Compustat fundamentals - ALL VRIABLES
# --------------------

if require_wrds_load==True:
    db = wrds.Connection(wrds_username='ngchunyue') # will ask for password if no .pgpass
...
print(f"Total unique vairables: {len(SAFE_ACCOUNTING_VARIABLES)}")

# Configuration: Choose which variables to process

# Option 1: Process only 4 vairables (current setup - for testing)
...
if require_wrds_load == True:
    ...
    try:
        # Use raw SQL quety to handle reserved words properly
        ...
        print(f"Successfully loaded Compustat data using raw SQL")
        ...
        try:
            ...
            print(f"Successfully loaded Compustat data using get_table")
    ...
    comp = pd.read_parquet(os.path.join(base_directory, "raw_compustat_data.parquet"), engine='fastparquet')
...
# Filter to only include variable that exist in the data and are in our signal list
...
# --------------------
if require_wrds_load == True:
    print(f"Saving raw data backup...")
    ...
    raw_data_backup.to_parquet(os.path.join(base_directory, "raw_compustat_data.parquet"), engine="fastparquet", index=False)

    print(f"Raw data saved: {raw_data_backup.shape[0]} rows, {raw_data_backup.shape[1]} columns")
    print("Saved to : raw_compustat.parquet")

else:
    print("Raw data backup already exists")

end_step("2.a Save raw data backup")
...
# Filter from START YEAR onwards
...
print("2b. Filter timeframe")

# --------------------
# 3. Mark adjacency (fyear_t - fyear_{t-1} within gvkey)
# --------------------
...
end_step("Compute denominator lag")

# --------------------
# 4. Save comp_base
# --------------------
print(f"Step 4. save comp_base")
...
comp.to_parquet(os.path.join(base_directory, f"comp_base_{START_YEAR}.parquet"), engine="fastparquet", index=False)
print(f"Saved comp_base_{START_YEAR}.parquet", comp.shape)

end_step("Save comp_base")
...
# comp_base_small.to_parquet(os.path.join(base_directory, f"comp_base_small_{START_YEAR}.parquet"), engine="fastparquet", index=False)
# print(f"Saved comp_base_small_{START_YEAR}.parquet", comp_base_small.shape)

# --------------------
# 5. Link Compustat (comp_base) to CRSP via CCM (SAS t4)
# --------------------
print("Step 1: Building CCM date-valid line (t4)...")

db = wrds.Connection(wrds_username='ngchunyue')

try:
    comp_base

except NameError:
    comp_base = pd.read_parquet(os.path.join(base_directory, f"comp_base_{START_YEAR}.parquet"))
...
end_step("5 — CCM link (t4)")

# %%
# ------------------------------
# 6. Build CRSP inputs and form t5/t6/t7/a0 (match SAS L51–L87, L431–L468)
# ------------------------------
...
end_step("6. CRSP inputs + t5/t6/t7/a0")

# ------------------------------
# 7. Merge price, add CRSP header (msenames), map FF48, apply SAS filters → a1/a2/a3/a33
# ------------------------------
...
end_step("7. price + msenames + FF48 + filters (a1/a2/a3/a33)")

# ------------------------------
# 8. Merge Compustat base (comp_base) with formation keys (a33)
# ------------------------------
print("8. Merging comp_base with a33 (forming observations only) and reporting drops ...")
...
comp_form.to_parquet(os.path.join(base_directory, f"comp_form_{START_YEAR}.parquet"), engine="fastparquet", index=False)
print(f"Saved comp_form_{START_YEAR}.parquet")

end_step("8. comp_base merged with a33 and drops reported")
...
cb_tix.to_csv(os.path.join(base_directory, "comp_base_tickers.csv"), index=False)
...
formed_tix.to_csv(os.path.join(base_directory, "formed_tickers.csv"), index=False)
...
comp_base = pd.read_parquet(os.path.join(base_directory, f"comp_base_{START_YEAR}.parquet"))
comp_form = pd.read_parquet(os.path.join(base_directory, f"comp_form_{START_YEAR}.parquet"))
...
# Full signals
signals_df.to_parquet(os.path.join(base_directory, f"signals_{START_YEAR}.parquet"), engine="fastparquet", index=False)

# Smaller preview
signals_df.head(1000).to_csv(os.path.join(base_directory, f"signals_{START_YEAR}_sample.csv"), index=False)

# Coverage diagnostics
coverage.to_csv(os.path.join(base_directory, f"coverage_{START_YEAR}.csv"), index=False)
...
signals_df = pd.read_parquet(os.path.join(base_directory, f"signals_{START_YEAR}.parquet"), engine="fastparquet")
...
forward_returns.to_parquet(os.path.join(base_directory, "forward_returns.parquet"), engine="fastparquet", index=False)
forward_returns.head(1000).to_csv(os.path.join(base_directory, "forward_returns_sample.csv"), index=False)

end_step("Step 11 — forward 12M returns")

# ------------------------------
# 12. Build 12-month risk-free (July→June cohorts) and merge into forward returns
# ------------------------------
...
forward_returns = pd.read_parquet(os.path.join(base_directory,"forward_returns.parquet"), engine="fastparquet")
...
# Persist
ret_with_rf.to_parquet(os.path.join(base_directory, "forward_returns_with_rf.parquet"), engine="fastparquet", index=False)
ret_with_rf.head(1000).to_csv(os.path.join(base_directory, "forward_returns_with_rf_sample.csv"), index=False)
...
signals_df = pd.read_parquet(os.path.join(base_directory, f"signals_{START_YEAR}.parquet"), engine="fastparquet")
comp_form = pd.read_parquet(os.path.join(base_directory, f"comp_form_{START_YEAR}.parquet"))
ret_with_rf = pd.read_parquet(os.path.join(base_directory, "forward_returns_with_rf.parquet"), engine="fastparquet")
...
merged.to_parquet(os.path.join(base_directory, out_name), engine="fastparquet", index=False)
merged.head(1000).to_csv(os.path.join(base_directory, f"signals_with_returns_{START_YEAR}_sample.csv"), index=False)
...
df = pd.read_parquet(os.path.join(base_directory, f"signals_with_returns_{START_YEAR}.parquet"), engine="fastparquet")
...
# Save diagnostics
cov_year.to_csv(os.path.join(base_directory, f"diagnostics_cov_by_year_{START_YEAR}.csv"), index=False)
nmonth_dist.to_frame("count").to_csv(os.path.join(base_directory, f"diagnostics_nmonth_dist_{START_YEAR}.csv"))
pd.Series(stats).to_csv(os.path.join(base_directory, f"diagnostics_basic_stats_{START_YEAR}.csv"))
...
out_final.to_parquet(os.path.join(base_directory, final_name), engine="fastparquet", index=False)
out_final.head(1000).to_csv(os.path.join(base_directory, f"signals_with_returns_formed_{START_YEAR}_sample.csv"), index=False)

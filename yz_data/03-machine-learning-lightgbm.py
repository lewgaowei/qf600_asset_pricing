...
output_dir = _base_dir / "ml_lightgbm_results"
print(_base_dir)
print(output_dir)
...
def get_hyperparameter_grid(method):
    ...
    if method == 'brt':
        ...
        grid = {
            ...
            'max_depth': [-1]                      # Tree depth (-1 = no limit, LightGBM default)
        }
    else:
...
def run_cross_validation(data, k, config, feature_cols):
    ...
    for i, params in tqdm(enumerate(tunegrid), total=len(tunegrid), desc="Testing Hyperparameters", unit="combo", leave=False):
        ...
        if config['method'] == 'brt':
            model = lgb.LGBMRegressor(
                ...
                random_state=42,
                device='cpu',  # CPU mode
                n_jobs=-1,  # Use all CPU cores (sequential mode is stable)
...
USE_PARALLEL = False  # ⚠️ Disabled due to ProcessPool crashes - use sequential mode for stability
...
if RUN_CV:
    ...
    if USE_PARALLEL:
        ...
        with ProcessPoolExecutor(max_workers=MAX_WORKERS) as executor:
            ...
            print(f"\n🔄 Processing {len(counters)} time periods in parallel...")
            with tqdm(total=len(counters), desc="Cross-Validation Progress", unit="period") as pbar:
                for future in as_completed(futures):
                    k, year, success, error = future.result()

                    if success:
                        pbar.set_postfix_str(f"✅ Counter {k+1} (Year {year:.0f})")
                    else:
                        pbar.set_postfix_str(f"❌ Counter {k+1} (Year {year:.0f}) - {error[:30]}")
        ...
        print(f"\n🔄 Processing {len(counters)} time periods sequentially...")
        for k in tqdm(counters, desc="Cross-Validation Progress", unit="period"):
            year = [y for y, c in year_to_counter.items() if c == k+1][0]  # k+1 is the test counter

            # Show current period info
            tqdm.write(f"\n{'='*60}")
            tqdm.write(f"Processing CV for test counter {k+1} (Year {year:.0f})")
            tqdm.write(f"  (Training: ≤{k-CONFIG['cv_validation']}, Validation: {k-CONFIG['cv_validation']+1}-{k})")
            tqdm.write(f"{'='*60}")

            try:
                cv_results = run_cross_validation(df_scaled, k, CONFIG, final_feature_columns)
                tqdm.write(f"✅ CV for test counter {k+1} complete!")

            except Exception as e:
                tqdm.write(f"❌ Error at test counter {k+1}: {str(e)}")
                tqdm.write("Continuing to next period...")
                continue

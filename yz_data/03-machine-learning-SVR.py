...
data_path = f"signals_with_returns_and_tickers_{START_YEAR}.parquet"
...
# "This is to standardise time progression by not letting the absolute value bias the model"
# ------------------------------
print("\n" + "=" * 60)
...
CONFIG = {
    ...
    # Model settings
    'method': 'svr',                # 'brt' = LightGBM boosted regression tree
...
print(f"Method:               {CONFIG['method'].upper()} (Support Vector Regression)")
...
if CONFIG['use_top_features'] is not None:
    ...
    for col in features_to_keep:
        # Get non-NaN pairs
        mask = df[col].notna() & df[CONFIG['dep_var']].notna()

        if mask.sum() < 100:  # Need at least 100 observations
            continue

        # Calculate Pearson correlation
        corr = df.loc[mask, col].corr(df.loc[mask, CONFIG['dep_var']])
...
final_col = final_feature_columns+base_columns
final_1000_df = df[final_col].copy()
#saving final feature data columns
final_data_file = _base_dir / f"final_1000_df_{START_YEAR}.parquet"
print(f"\n💾 Saving final 1000 columns data to: {final_data_file.name}")
final_1000_df.to_parquet(final_data_file, engine="fastparquet", compression="snappy")

...
def remove_multicollinear_features(data, feature_cols, dep_var, threshold=0.85):
    """
    Remove highly correlated features (multicollinearity removal).
    ...
    # Calculate feature-target correlations (for deciding which to keep)
    print("Calculating feature-target correlations...")
    target_correlations = {}
    for col in feature_cols:
        mask = data[col].notna() & data[dep_var].notna()
        if mask.sum() >= 100:
            corr = abs(data.loc[mask, col].corr(data.loc[mask, dep_var]))
            target_correlations[col] = corr if not pd.isna(corr) else 0
        else:

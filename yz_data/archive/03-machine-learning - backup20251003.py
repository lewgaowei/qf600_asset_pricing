...
CONFIG = {
    ...
    'cv_train': 5,                  # Years for training (only used in 'rolling' mode)
...
print(f"\nFor Test Period: Counter {k_example} (Year {year_example})")
print()

if CONFIG['window'] == 'recursive':
    # For test period k, we use:
    # Training: [min_counter, k - cv_validation]
    # Validation: [k - cv_validation + 1, k]
    # Test: k + 1
    # But k_example IS the test counter, so we need k = k_example - 1
    k_for_cv = k_example - 1
    ...
    # Same logic: k_example is test counter, so k = k_example - 1
    k_for_cv = k_example - 1
...
def get_hyperparameter_grid(method):
    ...
    if method == 'brt':
        ...
        grid = {
            'n_estimators': [20,60,100,200,400],  # Number of trees
...
def run_cross_validation(data, k, config, feature_cols):
    ...
    print(f"\n{'='*60}")
    print(f"Running Cross-Validation for Counter {k}")
    print(f"{'='*60}")
    ...
    print(f"Splitting data for counter {k}...")
    ...
    # Save results to CSV
    output_file = output_filename(config, mode='cv', counter=k)
...
if RUN_CV:
    ...
    if USE_PARALLEL:
        ...
        def cv_worker(k):
            """Worker function to run CV for a single counter"""
            try:
                year = [y for y, c in year_to_counter.items() if c == k][0]
        ...
        # Submit all jobs
        counters = list(range(CONFIG['begin'], CONFIG['end'] + 1))
        ...
        with ProcessPoolExecutor(max_workers=MAX_WORKERS) as executor:
            ...
            for future in as_completed(futures):
                ...
                if success:
                    print(f"✅ [{completed}/{len(counters)}] Counter {k} (Year {year:.0f}) complete!")
                else:
                    print(f"❌ [{completed}/{len(counters)}] Counter {k} (Year {year:.0f}) failed: {error}")

    else:
        # Sequential execution (original approach)
        for k in range(CONFIG['begin'], CONFIG['end'] + 1):
            year = [y for y, c in year_to_counter.items() if c == k][0]
            print(f"\n{'='*60}")
            print(f"Processing counter {k} (Year {year:.0f})")
            print(f"{'='*60}")

            try:
                cv_results = run_cross_validation(df_scaled, k, CONFIG, final_feature_columns)
                print(f"✅ Counter {k} complete!")

            except Exception as e:
                print(f"❌ Error at counter {k}: {str(e)}")

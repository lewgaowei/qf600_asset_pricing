...
from joblib import Parallel, delayed  # For CPU parallelism
# Suppress warnings for cleaner output
warnings.filterwarnings('ignore')

# ------------------------------
# GPU Acceleration Check (cuML for ElasticNet)
# ------------------------------
print("=" * 60)
print("GPU ACCELERATION CHECK (cuML for ElasticNet)")
print("=" * 60)

USE_GPU_CUML = False
try:
    import cuml
    print("✅ cuML library detected")

    # Try to enable GPU acceleration
    try:
        # Method 1: Jupyter/IPython magic
        get_ipython().run_line_magic('load_ext', 'cuml.accel')
        print("   GPU acceleration enabled via IPython magic")
        USE_GPU_CUML = True
    except (NameError, AttributeError):
        # Method 2: Direct import (for Python scripts)
        try:
            import cuml.accel
            cuml.accel.install()
            print("   GPU acceleration enabled via cuml.accel.install()")
            USE_GPU_CUML = True
        except Exception as e:
            print(f"   ⚠️  GPU acceleration failed: {str(e)[:50]}")

    if USE_GPU_CUML:
        print("\n🚀 ElasticNet will run on GPU (50x faster!)")
        print("   All sklearn ElasticNet calls automatically use NVIDIA GPU")
    else:
        print("\n💻 ElasticNet will use CPU (GPU init failed)")

except ImportError:
    print("⚠️  cuML not installed")
    print("   To enable GPU: pip install cuml-cu12")
    print("   ElasticNet will use CPU with parallelism")
except Exception as e:
    print(f"⚠️  cuML error: {str(e)[:100]}")
    print("   ElasticNet will use CPU")

if not USE_GPU_CUML:
    print(f"\n💻 CPU Mode: Will use joblib parallelism ({os.cpu_count()} cores)")
...
output_dir = _base_dir / "ml_elasticnet_results"
print(_base_dir)
print(output_dir)
...
CONFIG = {
    ...
    # Model settings
    'method': 'elasticnet',         # 'elasticnet' = Elastic Net regression
...
print(f"Method:               {CONFIG['method'].upper()} (Elastic Net)")
...
# %%
# ------------------------------
# STEP 4: Implement Hyperparameter Grid for Elastic Net
# ------------------------------
print("\n" + "=" * 60)
print("STEP 4: HYPERPARAMETER GRID FOR ELASTIC NET")
print("=" * 60)
...
def get_hyperparameter_grid(method):
    ...
    -----------
    method : str
        ML method ('elasticnet' for Elastic Net regression)
    ...
    if method == 'elasticnet':
        # EXPANDED Hyperparameters for Elastic Net (344 features, 10k-50k rows)
        # Comprehensive grid with parallelism: 48 combinations
        grid = {
            # Alpha: Regularization strength (8 values - logarithmic scale)
            'alpha': [0.0001, 0.0005, 0.001, 0.005, 0.01, 0.05, 0.1, 0.5, 1.0],

            # L1 ratio: Balance between L1 (Lasso) and L2 (Ridge) (6 values)
            'l1_ratio': [0.1, 0.3, 0.5, 0.7, 0.9, 0.95],

            # Max iterations: Sufficient for convergence
            'max_iter': [5000]
        }
        # Total: 9 × 6 × 1 = 54 combinations
        #
        # Grid Design Philosophy:
        # - alpha: Fine-grained logarithmic sampling (0.0001 to 1.0)
        #   * 0.0001-0.001: Very weak regularization (good for many features)
        #   * 0.01-0.1: Moderate regularization (balanced)
        #   * 0.5-1.0: Strong regularization (for noisy data)
        #
        # - l1_ratio: Comprehensive L1/L2 balance
        #   * 0.1: Mostly Ridge (all features, shrunk)
        #   * 0.3-0.5: Balanced (some feature selection)
        #   * 0.7-0.9: Mostly Lasso (aggressive feature selection)
        #   * 0.95: Nearly pure Lasso (maximum sparsity)
        #
        # With GPU/CPU parallelism, 54 combinations train in ~1-5 seconds!
    else:
...
if CONFIG['method'] == 'elasticnet':
    # Extract unique values for each parameter
    alpha_values = sorted(set([p['alpha'] for p in hyperparameter_grid]))
    l1_ratio_values = sorted(set([p['l1_ratio'] for p in hyperparameter_grid]))
    max_iter_values = sorted(set([p['max_iter'] for p in hyperparameter_grid]))

    print(f"  alpha (regularization):       {alpha_values}")
    print(f"  l1_ratio (L1/L2 balance):     {l1_ratio_values}")
    print(f"  max_iter (max iterations):    {max_iter_values}")

print()
print("First 5 combinations to try:")
print("-" * 50)
print("  #  |     alpha    | l1_ratio | max_iter")
print("-" * 50)

for i, params in enumerate(hyperparameter_grid[:5], 1):
    print(f"  {i:2d} | {params['alpha']:12.4f} | {params['l1_ratio']:8.2f} | {params['max_iter']:8d}")
...
print("-" * 50)
print()
print("1. alpha (Regularization Strength):")
print("   - Controls the overall penalty for coefficient magnitudes")
print("   - Higher alpha = Stronger regularization = Smaller coefficients")
print("   - Too high = Underfitting (model too simple)")
print("   - Too low = Overfitting (model too complex)")
print()
print("2. l1_ratio (L1/L2 Balance):")
print("   - Mix between L1 (Lasso) and L2 (Ridge) regularization")
print("   - 0.0 = Pure Ridge (all features kept, shrunk)")
print("   - 1.0 = Pure Lasso (feature selection, sparse)")
print("   - 0.5 = Equal mix of both")
print()
print("3. max_iter (Maximum Iterations):")
print("   - Maximum number of iterations for convergence")
print("   - Higher values allow more time for optimization")
print("   - Set to 5000 to ensure convergence")
...
def run_cross_validation(data, k, config, feature_cols):
    ...
    # CPU PARALLELISM: Train multiple combinations simultaneously
    if not USE_GPU_CUML and len(tunegrid) > 1:
        # CPU Mode: Use joblib for parallel processing
        print(f"🚀 Using CPU parallelism ({os.cpu_count()} cores)")

        def train_single_combo(params):
            ...
            return {**params, 'r2_score': r2, 'mse': mse}

        # Parallel execution
        results_list = Parallel(n_jobs=-1, verbose=5)(
            delayed(train_single_combo)(params) for params in tunegrid
        )
        ...
        avg_time = total_time / len(tunegrid)
        print(f"\n✅ Completed {len(tunegrid)} combinations in {total_time:.1f}s")
        print(f"   Average: {avg_time:.2f}s per combination")

    else:
        # GPU Mode: Sequential execution (GPU handles parallelism internally)
        if USE_GPU_CUML:
            print(f"🚀 Using GPU acceleration (cuML - sequential batch)")
        else:
            print(f"⚠️  Single combination - using sequential mode")
        ...
        for i, params in enumerate(tunegrid):
            ...
            if (i + 1) % 5 == 0 or (i + 1) == len(tunegrid):
                ...
                print(f"  Completed {i+1:2d}/{len(tunegrid)} | "
                      f"alpha={params['alpha']:.4f}, l1={params['l1_ratio']:.2f} | "
    ...
    print("-" * 60)
    print("Rank |     alpha    | l1_ratio | MSE       | R²")
    print("-" * 60)

    for rank, (idx, row) in enumerate(cv_results.head(3).iterrows(), 1):
        print(f"  {rank}  | {row['alpha']:12.4f} | {row['l1_ratio']:8.2f} | "
...
print("     a. Train Elastic Net model on TRAIN data")
...
def run_prediction(data, k, config, feature_cols):
    ...
    print(f"   l1_ratio:      {best_params['l1_ratio']:.3f}")
...
if len(cv_files) == 0:
    ...
    print("\n" + "-"*100)
    print(f"{'Year':<6} | {'Counter':<8} | {'Alpha':<10} | {'L1 Ratio':<10} | {'R²':<10} | {'MSE':<12}")
    print("-"*100)
    for idx, row in best_results.iterrows():
        print(f"{int(row['year']):<6} | {int(row['counter']):<8} | {row['alpha']:<10.4f} | "
    ...
    print("\n1. ALPHA (in best models):")
    alpha_counts = best_results['alpha'].value_counts().sort_index()
    for alpha, count in alpha_counts.items():
        pct = count / len(best_results) * 100
        print(f"   {alpha:.4f}: {count:2d} times ({pct:5.1f}%)")
    print(f"   Mean: {best_results['alpha'].mean():.4f}")

    print("\n2. L1_RATIO (in best models):")
    l1_counts = best_results['l1_ratio'].value_counts().sort_index()
    for l1, count in l1_counts.items():
        pct = count / len(best_results) * 100
        print(f"   {l1:.2f}: {count:2d} times ({pct:5.1f}%)")
    print(f"   Mean: {best_results['l1_ratio'].mean():.3f}")

    print("\n3. MAX_ITER (in best models):")
    iter_counts = best_results['max_iter'].value_counts().sort_index()
    for max_i, count in iter_counts.items():
        pct = count / len(best_results) * 100
        print(f"   {int(max_i):5d}: {count:2d} times ({pct:5.1f}%)")
    print(f"   Mean: {best_results['max_iter'].mean():.1f}")

    print("\n" + "="*80)
    print("TOP 10 BEST PERFORMING CONFIGURATIONS (by MSE, across all periods)")
    print("="*80)
    top10 = best_results.nsmallest(10, 'mse')[['year', 'counter', 'alpha', 'l1_ratio', 'max_iter', 'mse', 'r2_score']]
    ...
    print("\nBy ALPHA (lower MSE = better):")
    alpha_avg = full_data.groupby('alpha')['mse'].mean().sort_values(ascending=True)
    for alpha, mse in alpha_avg.items():
        print(f"   {alpha:.4f}: {mse:.6f}")

    print("\nBy L1_RATIO (lower MSE = better):")
    l1_avg = full_data.groupby('l1_ratio')['mse'].mean().sort_values(ascending=True)
    for l1, mse in l1_avg.items():
        print(f"   {l1:.2f}: {mse:.6f}")

    print("\nBy MAX_ITER (lower MSE = better):")
    iter_avg = full_data.groupby('max_iter')['mse'].mean().sort_values(ascending=True)
    for max_i, mse in iter_avg.items():
        print(f"   {int(max_i):5d}: {mse:.6f}")
    ...
    print(f"   Alpha:     {best_results['alpha'].mode()[0]:.4f} (wins {alpha_counts.max()} times)")
    print(f"   L1 Ratio:  {best_results['l1_ratio'].mode()[0]:.2f} (wins {l1_counts.max()} times)")
    print(f"   Max Iter:  {int(best_results['max_iter'].mode()[0])} (wins {iter_counts.max()} times)")

    print(f"\n📊 Most Consistent Hyperparameters (by avg MSE, lower is better):")
    print(f"   Alpha:     {alpha_avg.idxmin():.4f} (avg MSE: {alpha_avg.min():.6f})")
    print(f"   L1 Ratio:  {l1_avg.idxmin():.2f} (avg MSE: {l1_avg.min():.6f})")
    print(f"   Max Iter:  {int(iter_avg.idxmin())} (avg MSE: {iter_avg.min():.6f})")
    ...
    if num_strategies >= 2:
        ...
        if has_fixed:
            ...
            summary_rows.append({
                ...
                'alpha': best_params_dict.get('alpha', ''),
                'l1_ratio': best_params_dict.get('l1_ratio', ''),
                'max_iter': best_params_dict.get('max_iter', ''),
                # Test period
        ...
        print(f"Hyperparameters: alpha={best_params_dict.get('alpha', 'N/A')}, "
              f"l1_ratio={best_params_dict.get('l1_ratio', 'N/A')}, "
              f"max_iter={best_params_dict.get('max_iter', 'N/A')}")

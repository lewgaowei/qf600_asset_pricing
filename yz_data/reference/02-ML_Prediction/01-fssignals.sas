...
libname compstat "F:\Data\Compustat";
libname ccm "F:\Data\CCM";
libname ml "F:\MachineLearning\sasdata";
libname ff "F:\Data\FF factors";
run;


* process compustat data "funda";

data comp1;
	set compstat.funda;
	if year(datadate)>2017 then delete;
...
* ff 48 industry classifications from WRDS;
...
* ===================================================================;
* merge with CRSP data using the CCM Linktable;
* ====================================================================;
...
* Merge Compustat variables with CRSP december market cap;
...
* merge with next June market cap;
data t6;
	set t5;
	year=year+1;
run;
...
*save the annual fsignals;

data fsignal;
	set a33;
	length fsvariable $ 10;
	fsvariable="&variable";
	keep permno form_date crsp_mktcap_6 fsvariable
...
proc append data=fsignal base=out;
run;
...
data out;
	set out;
	mindex=year(form_date)*12+month(form_date);
run;
...
***************************************;
*Merge with stock next12month return (July-NextJune);
***************************************;
data msi;
	set crsp.msi;
	mindex=year(date)*12+month(date);
...
data msi1;
	...
	by obs;
	retain i;
	if first.obs then i=0;
	if month^=7 then i=i;
	if month=7 then i=i+1;
run;

proc means data=msi1 noprint;
	by i;
	var mindex;
	output out=m1(keep=i mindex_begin mindex_end) min=mindex_begin max=mindex_end;
run;
...
data msi2;
	set msi1;
	if i=0 then delete;
	keep i mindex mindex_begin mindex_end;
run;
...
data ret;
	...
	mindex=year(date)*12+month(date);
	logret=log(1+ret);
	if year(date)<1960 then delete;
	keep permno mindex date ret logret;
run;
...
data ret3;
	...
	ret=exp(logret)-1;
	nmonth=_freq_;
	mindex=mindex_begin-1;
	keep permno i ret nmonth mindex;
run;
...
data rf;
	...
	logrf=log(1+rf);
	mindex=year(date)*12+ month(date);
	keep rf mindex logrf;
run;

data rf;
	merge rf(in=in1) msi2(in=in2);
	by mindex;
	if in1 and in2;
run;
...
proc sort data=out;
	by fsvariable form_date permno ;
run;

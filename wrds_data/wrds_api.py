...
def wrds_annual_fundamentals():
    db = wrds.Connection(wrds_username="lewgaowei")


    sql_file_path = os.path.join(os.path.dirname(__file__), "wrds_annual_fundamentals.sql")
    ...
    ticker_filter = "AND comp_na_daily_all.funda.tic IN (" + ",".join([f"'{t}'" for t in sp500_tickers]) + ")"
    ...
    print(df.head())
    output_path = os.path.join(os.path.dirname(__file__), "wrds_annual_fundamentals.csv")
    df.to_csv(output_path, index=False)


def wrds_quarterly_fundamentals():
    db = wrds.Connection(wrds_username="lewgaowei")


    sql_file_path = os.path.join(os.path.dirname(__file__), "wrds_quarterly_fundamentals.sql")
    with open(sql_file_path, "r") as f:
        sql_base = f.read()
    # define tickers
    # tickers = ["AAPL", "TSLA"]
    ...
    # build filter dynamically
    ticker_filter = "AND comp_na_daily_all.fundq.tic IN (" + ",".join([f"'{t}'" for t in sp500_tickers]) + ")"
    ...
    print(df.head())
    output_path = os.path.join(os.path.dirname(__file__), "wrds_quarterly_fundamentals.csv")
    df.to_csv(output_path, index=False)


if __name__ == "__main__":
    wrds_annual_fundamentals()
    wrds_quarterly_fundamentals()